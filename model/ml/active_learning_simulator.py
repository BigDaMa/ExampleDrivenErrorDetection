...
def go_to_next_column_prob(id_next, diff_certainty):
	...
	for key in diff_certainty.keys():
		ids_col = id_next[key]
		sum_certainty[key] = np.sum(diff_certainty[key][ids_col])
		#sum_certainty[key] = np.mean(diff_certainty[key][ids_col])#bad
		#sum_certainty[key] = np.min(diff_certainty[key][ids_col])#bad
		#sum_certainty[key] = np.sum(np.square(diff_certainty[key][ids_col]))#bad

	import operator
	return min(sum_certainty.iteritems(), key=operator.itemgetter(1))[0]
...
for counter_with_another_name in range(100,150):
	...
	try:
		from datasets.SimulatorDataset.Simulator import Simulator
		dataSet = Simulator(counter_with_another_name)
	...
	for check_this in range(checkN):
		...
		f = open("/tmp/simulation_log/log_progress_" + str(counter_with_another_name) + ".csv",
				 'w+')
