...
for d in range(len(datasets)):
    for ndata in range(N_datasets):
        file_path = "/home/felix/ExampleDrivenErrorDetection/log_progress_"+ datasets[d].name +"_" + str(ndata)  +".csv"
        train_x[d], train_y[d] = read_csv1(file_path, None)
...
for ndata in range(N_datasets_test):
    file_path_test = "/home/felix/ExampleDrivenErrorDetection/log_progress_" + model_for_dataset.name + "_" + str(ndata) + ".csv"
...
rmse = np.sqrt(mean_squared_error(train_y_test, pred_y_test))
...
print "Total values"
print "R2: " + str(r2_score(f_avg, f_avg_pred))
print "MSE: " + str(mean_squared_error(f_avg, f_avg_pred))

rmse = np.sqrt(mean_squared_error(f_avg, f_avg_pred))
