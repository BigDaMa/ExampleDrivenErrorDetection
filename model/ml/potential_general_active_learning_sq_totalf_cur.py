...
def go_to_next_column_prob(column_id, tp_pred, fp_pred, fn_pred):
	#calc current total estimated f1-score

	tp_all_sum = np.sum(tp_pred.values())
	fp_all_sum = np.sum(fp_pred.values())
	fn_all_sum = np.sum(fn_pred.values())
	...
	for key_column in tp_pred.keys():
		estimated_possible_total_fscore[key_column] = (2 * (tp_all_sum + fn_pred[key_column])) / ((2 * (tp_all_sum + fn_pred[key_column])) + ((fp_all_sum - fp_pred[key_column]) + (fn_all_sum - fn_pred[key_column])))
...
def load_model(dataSet, classifier):
	...
	fp_model = pickle.load(open(potential_model_dir + "/fp_model" + dataset_log_files[dataSet.name] + "_" + classifier.name + ".p"))
	fn_model = pickle.load(
		open(potential_model_dir + "/fn_model" + dataset_log_files[dataSet.name] + "_" + classifier.name + ".p"))
...
for check_this in range(checkN):
	...
	for round in range(label_iterations * dataSet.shape[1]):
		...
		if rounds_per_column[column_id] == 1:
			...
			num_errors = 2
			train[column_id], train_target[column_id], train_chosen_ids[column_id] = create_user_start_data(
				feature_matrix.tocsr(), target_run,
			...
			if train[column_id] == None:
				if round < dataSet.shape[1] * number_of_round_robin_rounds:
					...
					column_id = go_to_next_column_prob(column_id, tp_pred, fp_pred, fn_pred)
				continue
			...
			train[column_id], train_target[column_id], certainty[column_id], train_chosen_ids[
				column_id] = create_next_data(train[column_id],
			...
			assert len(train_chosen_ids[column_id]) == train[column_id].shape[0]
		...
		if column_id in feature_array_all:
			if not run_round_robin:
				...
				if tp_model == None:
					tp_model, fp_model, fn_model = load_model(dataSet, classifier)
				...
				fp_pred[column_id] = fp_model.predict(mat_potential)
				fn_pred[column_id] = fn_model.predict(mat_potential)
