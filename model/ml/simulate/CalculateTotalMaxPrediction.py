...
import operator
import pickle
from ml.simulate.MaxPrediction.maxprediction import select_by_max_prediction_change
from ml.simulate.MaxPrediction.maxprediction import get_prediction_change
...
for d in range(10):
    ...
    for run in range(runs):
        for col in range(n):
            ...
            matrix_change_sum[col, run] = change_sum[col + n * run]
    ...
    best_sum_total_f[d], best_col_seq[d] = select_by_max_prediction_change(tensor_run, np.ones(n, dtype=int), total_f, col_seq, matrix_change_sum, future_steps, True) #Flight = 9, Blackoak 7, Hospital=5
