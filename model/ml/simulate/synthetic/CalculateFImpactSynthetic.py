...
dataSet.name = "BlackOakUppercase"


dataset = dataSet
future_steps = 6 #BlackOak = 7, Flights = 9
...
for a in range(10):
    for b in range(10):
        for d in range(10):
            file_path = "/home/felix/ExampleDrivenErrorDetection/progress_log_data/" + log_folder + "/log_progress_"+ "Synthetic" +"_" + str(d)  +".csv"
            x, fp, fn, tp = read_csv1(file_path, None)

            estimated_scores = get_estimated_tp_fp_fn(x, n, dataset, which_features_to_use)
            ...
            new_estimated_scores = estimated_scores[[a, b], :, :]


            # print tensor_run
            total_f, col_seq = select_by_round_robin(new_tensor_run, np.ones(new_n, dtype=int) * -1, [], [], new_n * 2, True)

            best_sum_total_f[d], best_col_seq[d] = select_by_estimated_max_f_impact(new_tensor_run, np.ones(new_n, dtype=int), total_f, col_seq, estimated_scores, future_steps, True) #Flight = 9, Blackoak 7, Hospital=5
