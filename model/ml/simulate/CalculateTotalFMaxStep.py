...
import operator
import pickle
from ml.simulate.optimal.optimal import find_max_total_f_new
from ml.simulate.optimal.optimal import find_max_total_f
...
def calculateStateFromColumnSequence(n, cols):
...
for d in range(10):
    ...
    best_col_seq[d] = col_seq
    best_sum_total_f[d] = total_f



    step_size = 4

    while len(best_col_seq[d]) < n*2 + future_steps:
        current_state = calculateStateFromColumnSequence(n, best_col_seq[d])
        print current_state

        if check_for_convergence:
            done = {}
            for col_i in range(n):
                column_f1 =calc_column_f1(tensor_run, np.ones(n, dtype=int), col_i)
                if column_f1 == 1.0:
                    done[col_i] = True

            best_sum_total_f[d], best_col_seq[d] = find_max_total_f_new(tensor_run, current_state, best_sum_total_f[d], best_col_seq[d], step_size, done, True) #Flight = 9, Blackoak 7, Hospital=5
        else:
            best_sum_total_f[d], best_col_seq[d] = find_max_total_f(tensor_run, current_state, best_sum_total_f[d], best_col_seq[d],
                                                                    step_size,
