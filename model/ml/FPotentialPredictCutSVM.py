...
def run_cross_validation(train, train_target, folds, scoring='r2'):
    cv_params = {'alpha': [10000.0, 1000.0, 100.0, 10.0, 1.0, 0.1, 0.01, 0.001, 0.0001],
                 'normalize': [False, True],
                 'fit_intercept': [False, True]}
    ind_params = {
    }

    optimized_GBM = GridSearchCV(Ridge(**ind_params),
                                 cv_params,
...
param = dict(our_params)  # or orig.copy()
...
final = Ridge(**param)
final.fit(np.matrix(train_x_n), np.array(train_y_n,dtype=float))
