...
def confusion_list(y_true, y_pred):
    cm = np.matrix(confusion_matrix(y_true, y_pred, labels=[False, True]), dtype=float)
    cm = cm / float(len(y_true))
    ...
    confusion_list = np.array([fn, tp, fp])

    '''
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)

    print "my precision: " + str(precision) + " calc: " + str(precision_score(y_true, y_pred, labels=[False, True]))
    print "my recall: " + str(recall) + " calc: " + str(recall_score(y_true, y_pred, labels=[False, True]))
    '''

    return confusion_list

def cv_confusion_matrix(X, y, folds, our_params):
    ...
    for train, test in cv_iter:
        ...
        confusion = confusion_list(y[test], y_pred)

        cms.append(confusion)
    return np.matrix(cms)
...
for check_this in range(checkN):
    ...
    for round in range(31 * dataSet.shape[1]):
        ...
        if round < dataSet.shape[1]:
            ...
            confusion_list_cross_val = cv_confusion_matrix(train[column_id], train_target[column_id], 7, our_params)
        ...
        if round >= dataSet.shape[1]:
            ...
            for foldi in range(len(confusion_list_cross_val)):
                for conf in range(3):
                    str_hist += "," + str(confusion_list_cross_val[foldi,conf])

            for conf in range(3):
                str_hist += "," + str(np.mean(confusion_list_cross_val[:,conf]))

            real_confusion_list = confusion_list(target, res)
            real_confusion_list_str= ""

            for conf in range(3):
                real_confusion_list_str += "," + str(real_confusion_list[conf])

            distinct_values_fraction = float(len(dataSet.dirty_pd[dataSet.dirty_pd.columns[column_id]].unique())) / float(dataSet.shape[0])

            f.write(str(distinct_values_fraction) + ',' + str(train[column_id].shape[0]) + ',' + str(certainty[column_id]) + ',' + str(np.min(np.absolute(y_pred[column_id] - 0.5))) + str_hist + real_confusion_list_str + '\n')
