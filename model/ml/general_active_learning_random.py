...
def go_to_next_random_column(error_indices):
    return error_indices[np.random.randint(len(error_indices))]
...
error_indices = np.where(errors_per_column > 0)[0]
print error_indices
    ...
    for round in range(label_iterations * dataSet.shape[1]):
        ...
        if not column_id in train:
            ...
            if train[column_id] == None:
                ...
                column_id = go_to_next_random_column(error_indices)
                continue
