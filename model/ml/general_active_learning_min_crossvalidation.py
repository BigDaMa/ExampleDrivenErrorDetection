...
def go_to_next_column_crossval(column_id, cross_val, train, dataSet):
    if len(cross_val) < dataSet.get_number_dirty_columns():#round_robin:
        column_id = column_id + 1
        if column_id == dataSet.shape[1]:
            column_id = 0
        return column_id
    else:
        crossval_array = np.ones(dataSet.shape[1], dtype=float)

        if np.all(cross_val.values() == 1.0):
            min_labels = 100000
            min_labels_index = -1

            for key in cross_val.keys():
            ...
            for key, value in cross_val.iteritems():
                crossval_array[key] = value

            min_certainty_index = np.argmin(crossval_array)
...
for check_this in range(checkN):
    ...
    for round in range(label_iterations * dataSet.shape[1]):
        ...
        if round >= dataSet.shape[1]:
            ...
            mean_cross_val[column_id] = np.mean(eval_scores)
        ...
        column_id = go_to_next_column_crossval(column_id, mean_cross_val, train, dataSet)
