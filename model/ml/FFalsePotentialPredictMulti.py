...
for d in range(len(datasets)):
    for ndata in [0]:
        file_path = "/home/felix/ExampleDrivenErrorDetection/progress_log_data/" + log_folder + "/log_progress_"+ datasets[d].name +"_" + str(ndata)  +".csv"
        train_x, train_y = read_csv1(file_path, None)

        print "train: " + str(train_x.shape[0])
